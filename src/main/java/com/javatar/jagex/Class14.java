...
final class Class14 {
	...
	static final void updateLocation() {
		try {
			Class15.inputStream.setBitAccess(8);
			int hasLocationUpdate = Class15.inputStream.readBits(1, (byte) 69);
			if (hasLocationUpdate != 0) {
				int locOpcode = Class15.inputStream.readBits(2, (byte) 69);
				if (locOpcode == 0) {
					ItemDefinition.anIntArray2797[Class38_Sub20_Sub15.anInt3064++] = 2047;
				} else if (locOpcode == 1) {
					int walkingDirection = Class15.inputStream.readBits(3, (byte) 69);
					Client.myPlayer.move(walkingDirection, false);
					int hasFlagUpdate = Class15.inputStream.readBits(1, (byte) 69);
					if (hasFlagUpdate == 1)
						ItemDefinition.anIntArray2797[Class38_Sub20_Sub15.anInt3064++] = 2047;
				} else if (locOpcode == 2) {
					int walkingDirection = Class15.inputStream.readBits(3, (byte) 69);
					Client.myPlayer.move(walkingDirection, true);
					int runningDirection = Class15.inputStream.readBits(3, (byte) 69);
					Client.myPlayer.move(runningDirection, true);
					int hasFlagUpdate = Class15.inputStream.readBits(1, (byte) 69);
					if (hasFlagUpdate == 1)
						ItemDefinition.anIntArray2797[Class38_Sub20_Sub15.anInt3064++] = 2047;
				} else if (locOpcode == 3) {
					int sceneY = Class15.inputStream.readBits(7, (byte) 69);
					int sceneX = Class15.inputStream.readBits(7, (byte) 69);
					int hasFlagUpdate = Class15.inputStream.readBits(1, (byte) 69);
					if (hasFlagUpdate == 1)
						ItemDefinition.anIntArray2797[Class38_Sub20_Sub15.anInt3064++] = 2047;
					Client.height = Class15.inputStream.readBits(2, (byte) 69);
					int isTeleporting = Class15.inputStream.readBits(1, (byte) 69);
					Client.myPlayer.updatePosition(sceneX, sceneY, isTeleporting == 1);
				}
			}
		} catch (RuntimeException runtimeexception) {
			throw Class38_Sub1
					.method607(runtimeexception, "wc.D(" + true + ')');
		}
	}
